...
def find_duplicate_patient(
    ...
    """
    Prevent duplicate registration inside
    the same hospital.
    """
...
def show_view_patient(patient):
    ...
    with st.container(
        ...
        patient_tab, personal_tab, contact_tab, hospital_tab = st.tabs(
            [
                "Patient",
                "Personal",
                "Contact",
...
def render_patient_form(
    ...
    if patient.get(
        ...
        for name, hospital_id in (
            hospital_options.items()
        ):
    ...
    with st.form(
        ...
        first_col, middle_col, last_col, suffix_col = st.columns(
            [2, 2, 2, 1]
        )
        ...
        dob_col, sex_col, civil_col = st.columns(
            3
        )
        ...
        civil_status = civil_col.selectbox(
            ...
            CIVIL_STATUS_OPTIONS,
            index=(
                CIVIL_STATUS_OPTIONS.index(
                    civil_start
                )
                if civil_start
                in CIVIL_STATUS_OPTIONS
                else 0
            ),
        )

        job_col, nation_col, religion_col = st.columns(
            3
        )

        occupation = job_col.text_input(
            "Occupation",
            value=patient.get(
                "occupation"
            )
        ...
        nationality = nation_col.text_input(
            "Nationality",
            value=patient.get(
                "nationality"
            )
        ...
        religion = religion_col.text_input(
            "Religion",
            value=patient.get(
                "religion"
            )
        ...
        contact_col, address_col = st.columns(
            [1, 2]
        )
        ...
        emergency_name_col, emergency_number_col = st.columns(
            2
        )
    ...
    details = {
        ...
        "emergency_contact_name":
            emergency_name.strip()
            or None,

        "emergency_contact_no":
            emergency_number.strip()
            or None,
    ...
    try:
        ...
        if is_edit:
            ...
            if duplicate:
                ...
                errors.error(
                    ...
                    "already exists: "
                    f"{patient_name(duplicate)} "
                    f"(code "
                    f"{duplicate.get('patient_code') or '—'})."
                )
        ...
        errors.error(
            f"Failed to save patient: {error}"
        )
...
def show_delete_patient_dialog(
    patient
):
    ...
    with st.container(
        ...
        if confirm_buttons(
            f"delete_patient_{patient['patient_id']}",
            "Delete patient",
        ):
            ...
            try:
                ...
                st.error(
                    f"Failed to delete patient: {error}"
                )
